...
void Udp::initialize(int stage) {
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        if (crcMode == CRC_COMPUTED) {
            ...
            auto crcInsertion = check_and_cast<UdpCrcInsertionHook*>(
                    moduleType->create("crcInsertion", this));
...
#endif
#ifdef INET_WITH_IPv6
            auto ipv6 = dynamic_cast<INetfilter*>(findModuleByPath(
                    "^.ipv6.ipv6"));
            if (ipv6 != nullptr)
...
void Udp::handleUpperCommand(cMessage *msg) {
    ...
    case UDP_C_SETOPTION: {
        ...
        case UDP_C_SETOPTION_UNBLOCK_MCAST_SRC: {
            ...
            for (size_t i = 0; i < cmd->getSourceListArraySize(); i++)
                sourceList.push_back(cmd->getSourceList(i));
            unblockMulticastSources(sd, ie, cmd->getMulticastAddr(),
                    sourceList);
            break;
        }
        ...
        case UDP_C_SETOPTION_SET_MCAST_SRC_FILTER: {
            auto cmd = check_and_cast<UdpSetMulticastSourceFilterCommand*>(
                    ctrl);
            NetworkInterface *ie = ift->getInterfaceById(cmd->getInterfaceId());
            std::vector<L3Address> sourceList;
            ...
            throw cRuntimeError(
                    ...
                    ctrl->getOptionCode(), ctrl->getClassName());
        }
        break;
    }
    ...
    default: {
        throw cRuntimeError(
                "Unknown command code (message kind) %d received from app",
                msg->getKind());
    }
    }
...
void Udp::bind(int sockId, const L3Address &localAddr, int localPort) {
    ...
    if (existing != nullptr && (!sd || !sd->reuseAddr || !existing->reuseAddr))
        throw cRuntimeError("bind: local address/port %s:%u already taken",
                localAddr.str().c_str(), localPort);

    if (sd) {
        if (sd->isBound)
            throw cRuntimeError("bind: socket is already bound (sockId=%d)",
                    sockId);
...
Udp::SockDesc* Udp::findFirstSocketByLocalAddress(const L3Address &localAddr,
        ushort localPort) {
...
Udp::SockDesc* Udp::createSocket(int sockId, const L3Address &localAddr,
        int localPort) {
    ...
    EV_INFO << "Socket created: here " << *sd << "\n"; //TODO debug changes
    return sd;
}
...
void Udp::connect(int sockId, const L3Address &remoteAddr, int remotePort) {
    ...
    if (remotePort <= 0 || remotePort > 65535)
        throw cRuntimeError("connect: invalid remote port number %d",
                remotePort);
...
void Udp::joinMulticastGroups(SockDesc *sd,
    ...
    for (uint32_t k = 0; k < multicastAddresses.size(); k++) {
        ...
        MulticastMembership *membership = sd->findMulticastMembership(
                multicastAddr, interfaceId);
        if (membership)
            throw cRuntimeError(
                    "UPD::joinMulticastGroups(): %s group on interface %s is already joined.",
                    multicastAddr.str().c_str(),
                    interfaceId == -1 ?
                            "*" :
        ...
        if (interfaceId != -1) {
            ...
            if (!ie)
                throw cRuntimeError("Interface id=%d does not exist",
                        interfaceId);
...
void Udp::addMulticastAddressToInterface(NetworkInterface *ie,
...
#ifdef INET_WITH_IPv4
        ie->getProtocolDataForUpdate<Ipv4InterfaceData>()->joinMulticastGroup(
                multicastAddr.toIpv4());
#endif // ifdef INET_WITH_IPv4
    } else if (multicastAddr.getType() == L3Address::IPv6) {
#ifdef INET_WITH_IPv6
        ie->getProtocolDataForUpdate<Ipv6InterfaceData>()->assignAddress(
                multicastAddr.toIpv6(), false, SimTime::getMaxTime(),
                SimTime::getMaxTime());
#endif // ifdef INET_WITH_IPv6
    } else
...
void Udp::leaveMulticastGroups(SockDesc *sd,
        const std::vector<L3Address> &multicastAddresses) {
    ...
    for (auto &multicastAddresse : multicastAddresses) {
        ...
        while (it != sd->multicastMembershipTable.end()) {
            ...
            if (membership->interfaceId != -1) {
                NetworkInterface *ie = ift->getInterfaceById(
                        membership->interfaceId);
                ie->changeMulticastGroupMembership(membership->multicastAddress,
                        oldFilterMode, membership->sourceList,
                        MCAST_INCLUDE_SOURCES, empty);
            } else {
                for (int j = 0; j < ift->getNumInterfaces(); ++j) {
                    NetworkInterface *ie = ift->getInterface(j);
                    if (ie->isMulticast())
                        ie->changeMulticastGroupMembership(
                                membership->multicastAddress, oldFilterMode,
                                membership->sourceList, MCAST_INCLUDE_SOURCES,
                                empty);
                }
            }
...
void Udp::blockMulticastSources(SockDesc *sd, NetworkInterface *ie,
        L3Address multicastAddress, const std::vector<L3Address> &sourceList) {
    ASSERT(ie && ie->isMulticast());
    ASSERT(multicastAddress.isMulticast());

    MulticastMembership *membership = sd->findMulticastMembership(
            multicastAddress, ie->getInterfaceId());
    if (!membership)
        throw cRuntimeError(
    ...
    if (changed) {
        ie->changeMulticastGroupMembership(multicastAddress,
                MCAST_EXCLUDE_SOURCES, oldSources, MCAST_EXCLUDE_SOURCES,
                excludedSources);
    }
}

void Udp::unblockMulticastSources(SockDesc *sd, NetworkInterface *ie,
        L3Address multicastAddress, const std::vector<L3Address> &sourceList) {
    ASSERT(ie && ie->isMulticast());
    ASSERT(multicastAddress.isMulticast());
...
void Udp::leaveMulticastSources(SockDesc *sd, NetworkInterface *ie,
        L3Address multicastAddress, const std::vector<L3Address> &sourceList) {
    ASSERT(ie && ie->isMulticast());
    ASSERT(multicastAddress.isMulticast());
...
void Udp::joinMulticastSources(SockDesc *sd, NetworkInterface *ie,
        L3Address multicastAddress, const std::vector<L3Address> &sourceList) {
    ASSERT(ie && ie->isMulticast());
    ASSERT(multicastAddress.isMulticast());
    ...
    if (changed) {
        ie->changeMulticastGroupMembership(multicastAddress,
                MCAST_INCLUDE_SOURCES, oldSources, MCAST_INCLUDE_SOURCES,
                includedSources);
    }
}

void Udp::setMulticastSourceFilter(SockDesc *sd, NetworkInterface *ie,
        L3Address multicastAddress, UdpSourceFilterMode filterMode,
    ...
    bool changed = membership->filterMode != filterMode
            ...
            || !equal(sourceList.begin(), sourceList.end(),
                    membership->sourceList.begin());
    if (changed) {
        std::vector<L3Address> oldSources(membership->sourceList);
        McastSourceFilterMode oldFilterMode =
                membership->filterMode == UDP_INCLUDE_MCAST_SOURCES ?
        ...
        McastSourceFilterMode newFilterMode =
                filterMode == UDP_INCLUDE_MCAST_SOURCES ?
                        MCAST_INCLUDE_SOURCES : MCAST_EXCLUDE_SOURCES;

        membership->filterMode = filterMode;
        membership->sourceList = sourceList;

        ie->changeMulticastGroupMembership(multicastAddress, oldFilterMode,
                oldSources, newFilterMode, sourceList);
    }
}
...
void Udp::handleUpperPacket(Packet *packet) {
    if (packet->getKind() != UDP_C_DATA)
        throw cRuntimeError(
                "Unknown packet command code (message kind) %d received from app",
                packet->getKind());
    ...
    if (interfaceReq == nullptr && destAddr.isMulticast()) {
        ...
        int interfaceId =
                (membership != sd->multicastMembershipTable.end()
                        && (*membership)->interfaceId != -1) ?
                        (*membership)->interfaceId :
                        sd->multicastOutputInterfaceId;
        if (interfaceId != -1)
    ...
    // FIXME quite south of the border
//    EV_INFO << "Dest. Type" << destAddr.getType() << "\n";
    EV_INFO << "Dest. Type" << "came here" << "\n";
    if (destAddr.getType() == L3Address::MODULEPATH)
        l3Protocol = &Protocol::tarpf; // TODO - added for TarpF Simulation - Meshify
    else
        l3Protocol = destAddr.getAddressType()->getNetworkProtocol();
    ...
    if (totalLength.get() > UDP_MAX_MESSAGE_SIZE)
        throw cRuntimeError("send: total UDP message size exceeds %u",
    ...
    EV_INFO << "Sending app packet " << packet->getName() << " over "
                   << l3Protocol->getName() << ".\n";
...
void Udp::insertCrc(const Protocol *networkProtocol,
        const L3Address &srcAddress, const L3Address &destAddress,
        const Ptr<UdpHeader> &udpHeader, Packet *packet) {
...
uint16_t Udp::computeCrc(const Protocol *networkProtocol,
        const L3Address &srcAddress, const L3Address &destAddress,
    ...
    pseudoHeader->setPacketLength(
            udpHeader->getChunkLength() + udpData->getChunkLength());
    ...
    if (networkProtocol == &Protocol::ipv4)
        ...
        throw cRuntimeError("Unknown network protocol: %s",
                networkProtocol->getName());
...
void Udp::processUDPPacket(Packet *udpPacket) {
    ...
    auto udpHeader = udpPacket->popAtFront<UdpHeader>(b(-1),
            Chunk::PF_ALLOW_INCORRECT);

    // simulate checksum: discard packet if it has bit error
    EV_INFO << "Packet " << udpPacket->getName()
                   << " received from network, dest port "
    ...
    auto hasIncorrectLength = totalLength < udpHeader->getChunkLength()
            || totalLength
                    > udpHeader->getChunkLength() + udpPacket->getDataLength();
    auto networkProtocol =
            udpPacket->getTag<NetworkProtocolInd>()->getProtocol();
    ...
    if (!isMulticast && !isBroadcast) {
        ...
        std::vector<SockDesc*> sds = findSocketsForMcastBcastPacket(destAddr,
                destPort, srcAddr, srcPort, isMulticast, isBroadcast);
        if (sds.empty()) {
            EV_WARN << "No socket registered on port " << destPort << "\n";
            udpPacket->setFrontOffset(udpHeaderPopPosition);
            processUndeliverablePacket(udpPacket);
...
bool Udp::verifyCrc(const Protocol *networkProtocol,
        const Ptr<const UdpHeader> &udpHeader, Packet *packet) {
    ...
    case CRC_DECLARED_CORRECT: {
        ...
        auto udpDataBytes = packet->peekDataAt(B(0),
                totalLength - udpHeader->getChunkLength(),
                Chunk::PF_ALLOW_INCORRECT);
    ...
    case CRC_COMPUTED: {
        if (udpHeader->getCrc() == 0x0000)
            ...
            auto totalLength = udpHeader->getTotalLengthField();
            auto udpData = packet->peekDataAt<BytesChunk>(B(0),
                    totalLength - udpHeader->getChunkLength(),
                    Chunk::PF_ALLOW_INCORRECT);
            ...
            return computedCrc == 0xFFFF && udpHeader->isCorrect()
                    && udpData->isCorrect();
        }
    }
    default:
...
Udp::SockDesc* Udp::findSocketForUnicastPacket(const L3Address &localAddr,
        ushort localPort, const L3Address &remoteAddr, ushort remotePort) {
    auto it = socketsByPortMap.find(localPort);
    if (it == socketsByPortMap.end())
        return nullptr;
    ...
    for (SockDescList::reverse_iterator it = list.rbegin(); it != list.rend();
            ++it) {
        SockDesc *sd = *it;
        if (sd->onlyLocalPortIsSet
                || ((sd->remotePort == -1 || sd->remotePort == remotePort)
                        && (sd->localAddr.isUnspecified()
                                || sd->localAddr == localAddr)
...
std::vector<Udp::SockDesc*> Udp::findSocketsForMcastBcastPacket(
        const L3Address &localAddr, ushort localPort,
        const L3Address &remoteAddr, ushort remotePort, bool isMulticast,
        bool isBroadcast) {
...
void Udp::processUndeliverablePacket(Packet *udpPacket) {
    ...
    char buff[80];
    snprintf(buff, sizeof(buff), "Port %d unreachable",
            udpHeader->getDestinationPort());
    ...
    udpPacket->trim();
    udpPacket->insertAtFront(
            udpPacket->getTag<NetworkProtocolInd>()->getNetworkProtocolHeader());
    ...
    if (protocol->getId() == Protocol::ipv4.getId()) {
        ...
        icmp->sendErrorMessage(udpPacket, inIe, ICMP_DESTINATION_UNREACHABLE,
        ...
        icmpv6->sendErrorMessage(udpPacket, ICMPv6_DESTINATION_UNREACHABLE,
                PORT_UNREACHABLE);
#endif // ifdef INET_WITH_IPv6
        delete udpPacket;
...
void Udp::sendUp(Ptr<const UdpHeader> &header, Packet *payload, SockDesc *sd,
    ...
    payload->addTagIfAbsent<TransportProtocolInd>()->setProtocol(
            &Protocol::udp);
    payload->addTagIfAbsent<TransportProtocolInd>()->setTransportProtocolHeader(
            header);
...
void Udp::processICMPv4Error(Packet *packet) {
    ...
    if (ipv4Header->getDontFragment() || ipv4Header->getFragmentOffset() == 0) {
        const auto &udpHeader = packet->peekAtFront<UdpHeader>(B(8),
                Chunk::PF_ALLOW_INCOMPLETE);
    ...
    if (udpHeaderAvailable) {
        SockDesc *sd = findSocketForUnicastPacket(localAddr, localPort,
                remoteAddr, remotePort);
        if (sd) {
            // send UDP_I_ERROR to socket
            EV_DETAIL << "Source socket is sockId=" << sd->sockId
                             << ", notifying.\n";
            sendUpErrorIndication(sd, localAddr, localPort, remoteAddr,
                    remotePort);
        } else {
...
void Udp::processICMPv6Error(Packet *packet) {
    ...
    const Ipv6FragmentHeader *fh =
            dynamic_cast<const Ipv6FragmentHeader*>(ipv6Header->findExtensionHeaderByType(
...
void Udp::sendUpErrorIndication(SockDesc *sd, const L3Address &localAddr,
        ushort localPort, const L3Address &remoteAddr, ushort remotePort) {
...
bool Udp::isCorrectPacket(Packet *packet,
    ...
    if (udpHeader->getTotalLengthField() < UDP_HEADER_LENGTH)
        ...
        if (l3AddressInd != nullptr && networkProtocolInd != nullptr)
            return verifyCrc(networkProtocolInd->getProtocol(), udpHeader,
                    packet);
        else
 ...
 */
static bool lessMembership(const Udp::MulticastMembership *first,
        const Udp::MulticastMembership *second) {
...
Udp::MulticastMembershipTable::iterator Udp::SockDesc::findFirstMulticastMembership(
        const L3Address &multicastAddress) {
    MulticastMembership membership;
    membership.multicastAddress = multicastAddress;
    membership.interfaceId = 0; // less than any other interfaceId

    auto it = lower_bound(multicastMembershipTable.begin(),
            multicastMembershipTable.end(), &membership, lessMembership);
    if (it != multicastMembershipTable.end()
            && (*it)->multicastAddress == multicastAddress)
        return it;
    else
        return multicastMembershipTable.end();
}

Udp::MulticastMembership* Udp::SockDesc::findMulticastMembership(
        const L3Address &multicastAddress, int interfaceId) {
    MulticastMembership membership;
    membership.multicastAddress = multicastAddress;
    membership.interfaceId = interfaceId;
...
void Udp::SockDesc::deleteMulticastMembership(MulticastMembership *membership) {
    multicastMembershipTable.erase(
            std::remove(multicastMembershipTable.begin(),
                    multicastMembershipTable.end(), membership),
...
INetfilter::IHook::Result UdpCrcInsertionHook::datagramPostRoutingHook(
        Packet *packet) {
    ...
    if (networkHeader->getProtocol() == &Protocol::udp) {
        ...
        Udp::insertCrc(networkProtocol, srcAddress, destAddress, udpHeader,
                packet);
...
bool Udp::MulticastMembership::isSourceAllowed(L3Address sourceAddr) {
    ...
    return (filterMode == UDP_INCLUDE_MCAST_SOURCES && it != sourceList.end())
            || (filterMode == UDP_EXCLUDE_MCAST_SOURCES
                    && it == sourceList.end());
}
...
Udp::SockDesc* Udp::getSocketById(int sockId) {
    auto it = socketsByIdMap.find(sockId);
    if (it == socketsByIdMap.end())
        throw cRuntimeError("socket id=%d doesn't exist (already closed?)",
                sockId);
    return it->second;
}
