...
L3Address L3AddressResolver::resolve(const char *s, int addrType) {
    ...
    if (!tryResolve(s, addr, addrType))
        throw cRuntimeError("L3AddressResolver: Cannot resolve address `%s'",
                s);
    return addr;
}

std::vector<L3Address> L3AddressResolver::resolve(std::vector<std::string> strs,
        int addrType) {
...
bool L3AddressResolver::tryParse(L3Address &result, const char *addr,
        int addrType) {
    Ipv6Address ipv6;
    MacAddress mac;
...
bool L3AddressResolver::tryResolve(const char *s, L3Address &result,
        int addrType) {
    // empty address
    result = L3Address();
    if (!s || !*s)
    ...
    EV_DEBUG << "module name: " << modname << endl;
    char c = *nextsep;
    EV_DEBUG << "switching char: " << c << endl;
    ...
    if (c)
        throw cRuntimeError(
                "L3AddressResolver: syntax error parsing address spec `%s'", s);

    // find module
    cModule *mod = cSimulation::getActiveSimulation()->findModuleByPath(
            modname.c_str());
    if (!mod)
        throw cRuntimeError("L3AddressResolver: module `%s' not found",
                modname.c_str());

    // check protocol
    EV_DEBUG << "protocol: " << protocol << endl;
    if (!protocol.empty()) {
        if (protocol == "ipv4")
            addrType = ADDR_IPv4;
    ...
    EV_DEBUG << "destnodename: " << destnodename << "\n ifname: " << ifname
                    << endl;

    if (!destnodename.empty()) {
        cModule *destnode =
                cSimulation::getActiveSimulation()->findModuleByPath(
                        destnodename.c_str());
        if (!destnode)
            throw cRuntimeError(
                    "L3AddressResolver: destination module `%s' not found",
                    destnodename.c_str());
        result = addressOf(mod, destnode, addrType);
    } else if (ifname.empty()) {
        result = addressOf(mod, addrType);
        EV_DEBUG << "result: " << result.str() << endl;
...
L3Address L3AddressResolver::addressOf(cModule *host, int addrType) {
    EV_DEBUG << "addressOf executed" << endl;
    IInterfaceTable *ift = interfaceTableOf(host);
    EV_DEBUG << ift->getFullPath() << endl;
    return getAddressFrom(ift, addrType);
}

L3Address L3AddressResolver::addressOf(cModule *host, const char *ifname,
        int addrType) {
    IInterfaceTable *ift = interfaceTableOf(host);
    ...
    throw cRuntimeError(
            "L3AddressResolver: no interface called `%s' in interface table of `%s'",
            ifname, host->getFullPath().c_str());
}

L3Address L3AddressResolver::addressOf(cModule *host, cModule *destmod,
        int addrType) {
    IInterfaceTable *ift = interfaceTableOf(host);
    ...
    throw cRuntimeError(
            "L3AddressResolver: no interface connected to `%s' module in interface table of `%s'",
            destmod->getFullPath().c_str(), host->getFullPath().c_str());
}

L3Address L3AddressResolver::getAddressFrom(IInterfaceTable *ift,
        int addrType) {
    L3Address ret;
    EV_DEBUG << "addrType: " << addrType << endl;
    bool netmask = addrType & ADDR_MASK;
    if ((addrType & ADDR_IPv4) && getIpv4AddressFrom(ret, ift, netmask)) {
        EV_DEBUG << "came here 1" << endl;
        return ret;
    }

    else if ((addrType & ADDR_IPv6) && getIpv6AddressFrom(ret, ift, netmask)) {
        EV_DEBUG << "came here 2" << endl;
        return ret;
    } else if ((addrType & ADDR_MAC) && getMacAddressFrom(ret, ift, netmask)) {
        EV_DEBUG << "came here 3" << endl;
        return ret;
    } else if ((addrType & ADDR_MODULEPATH)
            && getModulePathAddressFrom(ret, ift, netmask)) {
        EV_DEBUG << "came here 4" << endl;
        return ret;
    } else if ((addrType & ADDR_MODULEID)
            && getModuleIdAddressFrom(ret, ift, netmask)) {
        EV_DEBUG << "came here 5" << endl;
        return ret;
    }
    EV_DEBUG << "No switch case triggered" << endl;
    return ret;
}

L3Address L3AddressResolver::getAddressFrom(NetworkInterface *ie,
        int addrType) {
    L3Address ret;
    ...
    if ((addrType & ADDR_IPv4) && getInterfaceIpv4Address(ret, ie, mask))
        ...
        return ret;
            ...
            && getInterfaceModulePathAddress(ret, ie, mask))
        return ret;
    else if ((addrType & ADDR_MODULEID)
            && getInterfaceModuleIdAddress(ret, ie, mask))
        return ret;
    return ret;
}

bool L3AddressResolver::getIpv4AddressFrom(L3Address &retAddr,
        IInterfaceTable *ift, bool netmask) {
    if (ift->getNumInterfaces() == 0)
        throw cRuntimeError(
                "L3AddressResolver: interface table `%s' has no interface registered "
                        "(yet? try in a later init stage!)",
                ift->getFullPath().c_str());
...
bool L3AddressResolver::getIpv6AddressFrom(L3Address &retAddr,
        IInterfaceTable *ift, bool netmask) {
    ...
    for (int i = 0;
            i < ift->getNumInterfaces() && retScope != Ipv6Address::GLOBAL;
            i++) {
        NetworkInterface *ie = ift->getInterface(i);
        L3Address curAddr;
...
bool L3AddressResolver::getMacAddressFrom(L3Address &retAddr,
        IInterfaceTable *ift, bool netmask) {
    if (ift->getNumInterfaces() == 0)
        throw cRuntimeError(
                "L3AddressResolver: interface table `%s' has no interface registered "
...
bool L3AddressResolver::getModulePathAddressFrom(L3Address &retAddr,
        IInterfaceTable *ift, bool netmask) {
    if (ift->getNumInterfaces() == 0)
        throw cRuntimeError(
                "L3AddressResolver: interface table `%s' has no interface registered "
    ...
    for (int i = 0; i < ift->getNumInterfaces(); i++) {
        ...
        if (getInterfaceModulePathAddress(retAddr, ie, netmask)){
            EV_DEBUG << "getInterfaceModulePathAddress returned true" << endl ;
            return true;
        }
...
bool L3AddressResolver::getModuleIdAddressFrom(L3Address &retAddr,
        IInterfaceTable *ift, bool netmask) {
    if (ift->getNumInterfaces() == 0)
        throw cRuntimeError(
                "L3AddressResolver: interface table `%s' has no interface registered "
...
bool L3AddressResolver::getInterfaceIpv6Address(L3Address &ret,
        NetworkInterface *ie, bool netmask) {
#ifdef INET_WITH_IPv6
    if (netmask)
        return false; // Ipv6 netmask not supported yet
...
bool L3AddressResolver::getInterfaceIpv4Address(L3Address &ret,
        NetworkInterface *ie, bool netmask) {
#ifdef INET_WITH_IPv4
    ...
    if (!addr.isUnspecified()) {
        ...
        Ipv4NetworkConfigurator *configurator =
                dynamic_cast<Ipv4NetworkConfigurator*>(cSimulation::getActiveSimulation()->findModuleByPath(
                        "configurator"));
        if (configurator)
            return static_cast<L3AddressResolver*>(configurator)->getInterfaceIpv4Address(
                    ret, ie, netmask);
    }
#endif // ifdef INET_WITH_IPv4
    return false;
...
bool L3AddressResolver::getInterfaceMacAddress(L3Address &ret,
        NetworkInterface *ie, bool netmask) {
...
bool L3AddressResolver::getInterfaceModulePathAddress(L3Address &ret,
        NetworkInterface *ie, bool netmask) {
#ifdef INET_WITH_NEXTHOP
    if (auto nextHopData = ie->findProtocolData<NextHopInterfaceData>()) {
        ...
        NextHopNetworkConfigurator *configurator =
                dynamic_cast<NextHopNetworkConfigurator*>(cSimulation::getActiveSimulation()->findModuleByPath(
                        "configurator"));
        if (configurator)
            return static_cast<L3AddressResolver*>(configurator)->getInterfaceIpv4Address(
                    ret, ie, netmask);
...
#endif // ifdef INET_WITH_NEXTHOP
    EV_DEBUG << "Macro excluded" << endl;
    ret = ie->getModulePathAddress();
    EV_DEBUG << "Is unspecified: " <<  ret.isUnspecified() << endl ;
    return true;
}

bool L3AddressResolver::getInterfaceModuleIdAddress(L3Address &ret,
        NetworkInterface *ie, bool netmask) {
#ifdef INET_WITH_NEXTHOP
    if (auto nextHopData = ie->findProtocolData<NextHopInterfaceData>()) {
        L3Address addr = nextHopData->getAddress();
...
IInterfaceTable* L3AddressResolver::interfaceTableOf(cModule *host) {
    ...
    if (!mod)
        throw cRuntimeError(
                "L3AddressResolver: IInterfaceTable not found as submodule "
...
IIpv4RoutingTable* L3AddressResolver::getIpv4RoutingTableOf(cModule *host) {
    ...
    if (!mod)
        throw cRuntimeError(
                "L3AddressResolver: IIpv4RoutingTable not found as submodule "
...
#ifdef INET_WITH_IPv4
    return dynamic_cast<IIpv4RoutingTable*>(host->findModuleByPath(
            ".ipv4.routingTable"));
#else // ifdef INET_WITH_IPv4
    return nullptr;
#endif // ifdef INET_WITH_IPv4
...
#ifdef INET_WITH_IPv6
    return dynamic_cast<Ipv6RoutingTable*>(host->findModuleByPath(
            ".ipv6.routingTable"));
...
NextHopRoutingTable* L3AddressResolver::findNextHopRoutingTableOf(
        cModule *host) {
#ifdef INET_WITH_NEXTHOP
    return dynamic_cast<NextHopRoutingTable*>(host->findModuleByPath(
...
std::vector<cModule*> L3AddressResolver::collectNetworkNodes() {
    ...
    doCollectNetworkNodes(cSimulation::getActiveSimulation()->getSystemModule(),
            result);
    return result;
}

void L3AddressResolver::doCollectNetworkNodes(cModule *parent,
        std::vector<cModule*> &result) {
...
NetworkInterface* L3AddressResolver::findInterfaceWithMacAddress(
